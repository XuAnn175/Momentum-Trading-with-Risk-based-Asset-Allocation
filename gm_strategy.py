...
class GMPositionSizer:
    ...
    def load_init_data(self, data):
        # data is a dataframe
        self.price_history = data["Adj Close"].tolist()
    ...
    def get_optimal_position_size(self, signal: str, current_price: float, 
        ...
        if signal == "buy":
            ...
            best_option = self._find_best_sell_option(current_price, predicted_price)
        else:
            return None
    ...
    def _find_best_sell_option(self, current_price: float, predicted_price: float) -> dict:
...
if __name__ == "__main__":
    gm_strategy = GMPositionSizer(symbol="SPOT_BTC_USDT", history_length=10, mu=0.5, trading_options=[0.001, 0.002, 0.003], min_balance=100)
    gm_strategy.get_optimal_position_size(signal="buy", current_price=100, available_balance=1000)
